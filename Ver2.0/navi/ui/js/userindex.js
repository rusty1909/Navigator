	...
	function fetchBillDetails(id){
        ...
        jQuery.ajax({
            type: 'POST',
            url: '../../../utility/helper/User/UserActionHelper.php?action=billdetail&id='+id,
            cache: false,
            success: function(response){
				if(response == 0){
					...
					var vehicle_image = '../../../images/vehicle_types/'+billDetail.vehicle.type+'.png';
					...
					var driver_image = '../../../images/driver_icon.png';
					...
					var bill_image_path = '../../../images/bills/'+billDetail.vehicle.number+'/'+billDetail.filename+'.jpg';
					var bill_image_text = "<img width='100' src='"+bill_image_path+"'>";
					document.getElementById("bill_image").innerHTML = bill_image_text;
					
					var reason_image = '../../../images/info.png';
	...
	function approveBill(isApproval) {
		jQuery.ajax({
            type: 'POST',
            url: '../../../utility/helper/User/UserActionHelper.php?action=billapproval&id='+bill_id+'&approval='+isApproval,
            cache: false,
            success: function(response){
				if(response == 0){
				}
		...
		geocoder.geocode({'location': latlng}, function(results, status) {
			if (status == google.maps.GeocoderStatus.OK) {
				if (results[1]) {
					...
					var calendar_image = '../../../images/location_icon.png';
	...
	function fetchNotification(){
        ...
        jQuery.ajax({
            type: 'POST',
            url: '../../../utility/helper/User/notification.php',
            cache: false,
            success: function(response){
				if(response == 0){
				}
